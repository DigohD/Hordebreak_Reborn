...
namespace FNZ.Shared.Utils.CollisionUtils
{
    ...
    public class FNECollisionUtils
    {
        ...
        public static FNERayCastHitStruct RayCastForEnemiesInModel(float2 start, float2 end, GameWorldInstance world)
        {
            ...
            GetBresenhamTilesFromLine3Wide((int) start.x, (int) start.y, (int) end.x, (int) end.y, world,
                raycastHitList);

            for (int i = raycastHitList.Count - 1; i >= 0; i--)
            {
                if (world.GetTile(raycastHitList[i]) == null)
                {
            ...
            {
                var tileEnemies = world.GetEnemiesOnTile(tilePos);
                if (tileEnemies != null)
        ...
        public static FNERayCastHitStruct RayCastForPlayersInModel(float2 start, float2 end, GameWorldInstance world,
            long playerToIgnore)
        {
            List<int2> raycastHitList = PopRayCastHitList();
            ...
            foreach (var tilePos in raycastHitList)
            {
                var tilePlayers = world.GetPlayersOnTile(tilePos);
                if (tilePlayers != null)
        ...
        public static List<FNERayCastHitStruct> RayCastAllEnemiesInModel(float2 start, float2 end,
            GameWorldInstance world, float widthMultiplier = 1f)
        {
            List<int2> raycastHitList = PopRayCastHitList();
            ...
            if (world.GetTile(start) != null)
            {
                ...
                foreach (var tilePos in raycastHitList)
                {
                    var tile = world.GetTile(tilePos);
                    if (tile == null) continue;
                    var tileEnemies = tile.GetEnemies();
                    foreach (var e in tileEnemies)
                    {
                        float radius = e.Data.GetComponentData<EnemyStatsComponentData>().hitboxRadius *
                                       widthMultiplier;
        ...
        public static FNERayCastHitStruct ExcavatorRayCastForWallsAndTileObjectsInModel(float2 start, float2 end,
            GameWorldInstance world)
        {
            List<int2> raycastHitList = PopRayCastHitList();
            ...
            if (world.GetTile(start) != null)
            {
                ...
                foreach (var tilePos in raycastHitList)
                {
                    ...
                    if (tileEntity != null && tileEntity.HasComponent<ExcavatableComponentShared>() &&
                        tileEntity.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(tileEntity);
                    }

                    var southEdgeObject =
                        world.GetEdgeObject(tilePos.x, tilePos.y, GameWorld.EdgeObjectDirection.SOUTH);
                    if (southEdgeObject != null && southEdgeObject.HasComponent<ExcavatableComponentShared>() &&
                        southEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                    ...
                    if (westEdgeObject != null && westEdgeObject.HasComponent<ExcavatableComponentShared>() &&
                        westEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(westEdgeObject);
                    }

                    var northEdgeObject =
                        world.GetEdgeObject(tilePos.x, tilePos.y + 1, GameWorld.EdgeObjectDirection.SOUTH);
                    if (northEdgeObject != null && northEdgeObject.HasComponent<ExcavatableComponentShared>() &&
                        northEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(northEdgeObject);
                    }

                    var eastEdgeObject =
                        world.GetEdgeObject(tilePos.x + 1, tilePos.y, GameWorld.EdgeObjectDirection.WEST);
                    if (eastEdgeObject != null && eastEdgeObject.HasComponent<ExcavatableComponentShared>() &&
                        eastEdgeObject.HasComponent<PolygonComponentShared>())
                    {
        ...
        public static FNERayCastHitStruct ProjectileRayCastForWallsAndTileObjectsInModel(float2 start, float2 end,
            GameWorldInstance world)
        {
            List<int2> raycastHitList = PopRayCastHitList();
            ...
            var entitiesToCheck = new List<FNEEntity>();
            ...
            if (tile != null)
            {
                ...
                {
                    var tileEntity = world.GetTileObject(tilePos.x, tilePos.y);
                    if (tileEntity != null && tileEntity.GetComponent<TileObjectComponentShared>().hittable &&
                        tileEntity.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(tileEntity);
                    }
                    ...
                    var southEdgeObject =
                        world.GetEdgeObject(tilePos.x, tilePos.y, GameWorld.EdgeObjectDirection.SOUTH);
                    if (southEdgeObject != null &&
                        southEdgeObject.GetComponent<EdgeObjectComponentShared>().IsHittable &&
                        southEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(southEdgeObject);
                    }
                    ...
                    var westEdgeObject = world.GetEdgeObject(tilePos.x, tilePos.y, GameWorld.EdgeObjectDirection.WEST);
                    if (westEdgeObject != null && westEdgeObject.GetComponent<EdgeObjectComponentShared>().IsHittable &&
                        westEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(westEdgeObject);
                    }
                    ...
                    var northEdgeObject =
                        world.GetEdgeObject(tilePos.x, tilePos.y + 1, GameWorld.EdgeObjectDirection.SOUTH);
                    if (northEdgeObject != null &&
                        northEdgeObject.GetComponent<EdgeObjectComponentShared>().IsHittable &&
                        northEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(northEdgeObject);
                    }
                    ...
                    var eastEdgeObject =
                        world.GetEdgeObject(tilePos.x + 1, tilePos.y, GameWorld.EdgeObjectDirection.WEST);
                    if (eastEdgeObject != null && eastEdgeObject.GetComponent<EdgeObjectComponentShared>().IsHittable &&
                        eastEdgeObject.HasComponent<PolygonComponentShared>())
                    {
                        entitiesToCheck.Add(eastEdgeObject);
                    }
            ...
            float circleRadius, ref LineCircleCollisionResult result)
        {
            ...
            float hitDist = proj.magnitude -
                            Mathf.Sqrt(circleRadius * circleRadius - hitNormal.magnitude * hitNormal.magnitude);
            ...
            if ((v).magnitude >= hitDist &&
                (Vector2.Dot(proj, v) > 0 || (start - circlePos).magnitude <= circleRadius) &&
                hitNormal.magnitude <= circleRadius)
            {
                result.hit = true;
        ...
        public static PolygonCircleHitResult PolygonCircleCollision(FNEPolygon polygon, Vector2 circlePos, float r,
            Vector2 relativeCircleVelocity)
        {
        ...
        public static bool get_line_intersection(float line1start_x, float line1start_y, float line1end_x,
            float line1end_y,
            float line2start_x, float line2start_y, float line2end_x, float line2end_y, out float hit_x,
            out float hit_y)
        {
            float s1_x, s1_y, s2_x, s2_y;
            s1_x = line1end_x - line1start_x;
            s1_y = line1end_y - line1start_y;
            s2_x = line2end_x - line2start_x;
            s2_y = line2end_y - line2start_y;

            float s, t;
            s = (-s1_y * (line1start_x - line2start_x) + s1_x * (line1start_y - line2start_y)) /
                (-s2_x * s1_y + s1_x * s2_y);
            t = (s2_x * (line1start_y - line2start_y) - s2_y * (line1start_x - line2start_x)) /
                (-s2_x * s1_y + s1_x * s2_y);
        ...
        public static void GetBresenhamTilesFromLine3Wide(int x, int y, int x2, int y2, GameWorldInstance world,
            List<int2> output)
        {
        ...
        private static void AddTileIfNotNull(GameWorldInstance world, int2 tilePos, List<int2> output)
        {
        ...
        public static void GetBresenhamTilesFromLine(int x, int y, int x2, int y2, GameWorldInstance world,
            List<int2> output)
        {
            int w = x2 - x;
            int h = y2 - y;
        ...
        public static bool Intersects(FNECircle circle, FNERectangle rect)
        {
            ...
            if (circleDistanceX > (rect.Width / 2.0f + circle.Radius))
            {
                return false;
            }

            if (circleDistanceY > (rect.Height / 2.0f + circle.Radius))
            {
                return false;
            }

            if (circleDistanceX <= (rect.Width / 2.0f))
            {
                return true;
            }

            if (circleDistanceY <= (rect.Height / 2.0f))
            {
                return true;
            }
