...
namespace FNZ.Server.Net.API
{
    ...
    {
        public void WorldEvent_SpawnWorldEventToPlayersInRange(INetSerializeableData eventData, ServerWorld world, float2 position, float radius)
        {
            var message = m_WorldEventMessageFactory.CreateWorldEventMessage(eventData);
            Broadcast_All_InProximity(world, message, position, radius);
        }
