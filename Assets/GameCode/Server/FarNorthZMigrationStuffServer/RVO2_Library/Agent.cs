	...
	public class Agent
	{
		...
		internal void computeNeighbors()
		{
			try
			{
				...
				computeObstacleNeighboursNew(this, ref rangeSq);
				...
				if (maxNeighbors_ > 0)
				{
					...
					computeAgentNeighboursNew(this, ref rangeSq);
				}
			}
			catch(Exception e)
			{
		...
		internal void computeObstacleNeighboursNew(Agent agent, ref float rangeSq)
		{
			int2 currentTile = new int2((int)agent.position_.x, (int)agent.position_.y);
			int2 southTile = new int2(currentTile.x, currentTile.y - 1);
			int2 westTile = new int2(currentTile.x - 1, currentTile.y);
			int2 northTile = new int2(currentTile.x, currentTile.y + 1);
			int2 eastTile = new int2(currentTile.x + 1, currentTile.y);
			int2 southwestTile = new int2(currentTile.x - 1, currentTile.y - 1);
			int2 northwestTile = new int2(currentTile.x - 1, currentTile.y + 1);
			int2 northeastTile = new int2(currentTile.x + 1, currentTile.y + 1);
			int2 southeastTile = new int2(currentTile.x + 1, currentTile.y - 1);

			ComputeObstacle(currentTile, agent, rangeSq);
			ComputeObstacle(southTile, agent, rangeSq);
			ComputeObstacle(westTile, agent, rangeSq);
			ComputeObstacle(northTile, agent, rangeSq);
			ComputeObstacle(eastTile, agent, rangeSq);
			ComputeObstacle(southwestTile, agent, rangeSq);
			ComputeObstacle(northwestTile, agent, rangeSq);
			ComputeObstacle(northeastTile, agent, rangeSq);
			ComputeObstacle(southeastTile, agent, rangeSq);

			FNEEntity southWall = World.GetEdgeObject(currentTile.x, currentTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH);
			FNEEntity westWall = World.GetEdgeObject(currentTile.x, currentTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST);
			FNEEntity northWall = World.GetEdgeObject(northTile.x, northTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH);
			FNEEntity eastWall = World.GetEdgeObject(eastTile.x, eastTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST);
			...
			ComputeWallObstacle(World.GetEdgeObject(southTile.x, southTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(westTile.x, westTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(northwestTile.x, northwestTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(northTile.x, northTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(northeastTile.x, northeastTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(northeastTile.x, northeastTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(eastTile.x, eastTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.SOUTH), agent, rangeSq);
			ComputeWallObstacle(World.GetEdgeObject(southeastTile.x, southeastTile.y, FNZ.Shared.Model.World.GameWorld.EdgeObjectDirection.WEST), agent, rangeSq);
		}

		private bool ComputeObstacle(int2 tile, Agent agent, float rangeSq)
		{
			var tileObject = World.GetTileObject(tile.x, tile.y);

			if (tileObject != null)
			{
				ObstacleComponentServer oc = tileObject.GetComponent<ObstacleComponentServer>();
				if (oc == null) return false;

				Obstacle currentObs = oc.obstacle;
				bool shouldAdd = true;

				foreach (var pair in agent.obstacleNeighbors_)
				{
					if (pair.Value == currentObs)
					{
						shouldAdd = false;
						break;
					}
				...
				if (shouldAdd)
				{
					addObstacleNeighbor(currentObs, agent, rangeSq);
				}
			}
		...
		private void ComputeWallObstacle(FNEEntity wall, Agent agent, float rangeSq)
		{
			if (wall == null) return;

			var comp = wall.GetComponent<EdgeObjectComponentServer>();
			if (comp == null)
			{
				Debug.Log($"Entity with id: {wall.Data.Id} has no EdgeObjectComponentServer");
				return;
			}
			if (comp.IsDestroyed || comp.IsPassable) return;

			ObstacleComponentServer oc = wall.GetComponent<ObstacleComponentServer>();
			if (oc == null || !oc.isInSimulation) return;
			Obstacle o = oc.obstacle;
			bool shouldAdd = true;
			...
			if (shouldAdd)
			{
				addObstacleNeighbor(o, agent, rangeSq);
			}
		}

		private void addObstacleNeighbor(Obstacle obs, Agent agent, float rangeSq)
		{
			for (int i = 0; i < 4; i++)
			{
				float agentLeftOfLine = RVOMath.leftOf(obs.point_, obs.next_.point_, agent.position_);
				float distSqLine = RVOMath.sqr(agentLeftOfLine) /
					RVOMath.absSq(obs.next_.point_ - obs.point_);

				if (distSqLine < rangeSq)
				{
					if (agentLeftOfLine < 0.0f)
					{
						agent.insertObstacleNeighbor(obs, rangeSq);
					}
				}
		...
		internal void computeAgentNeighboursNew(Agent agent, ref float rangeSq)
		{
			int2 currentTile = new int2((int)agent.position_.x, (int)agent.position_.y);
			...
			ComputeAgentNeighbour(currentTile, rangeSq);
			ComputeAgentNeighbour(south, rangeSq);
			ComputeAgentNeighbour(west, rangeSq);
			ComputeAgentNeighbour(north, rangeSq);
			ComputeAgentNeighbour(east, rangeSq);
			ComputeAgentNeighbour(southwest, rangeSq);
			ComputeAgentNeighbour(northwest, rangeSq);
			ComputeAgentNeighbour(northeast, rangeSq);
			ComputeAgentNeighbour(southeast, rangeSq);
		}
		...
		private void ComputeAgentNeighbour(int2 tile, float rangeSq)
		{
			...
			var cc = World.GetChunkCellData(tile.x, tile.y);
			if (cc == null) return;
			...
			if (tileEnemies != null)
			{
				foreach (var enemy in tileEnemies)
				{
					...
					int agentID = enemy.Agent.agentID;
					if (agentID < 0 || agentID >= Simulator.Instance.agents_.Count)
					{
						Debug.LogError($"What the frick!?, agentId: {agentID} is out of range for enemy with netId: {enemy.NetId}");
						continue;
					}
					Agent a = Simulator.Instance.agents_[agentID];
					insertAgentNeighbor(a, ref rangeSq);
				}
			}
		}
		...
		internal void computeNewVelocity()
		{
			...
			for (int i = 0; i < obstacleNeighbors_.Count; ++i)
			{
				...
				{
					if (RVOMath.det(invTimeHorizonObst * relativePosition1 - orcaLines_[j].point, orcaLines_[j].direction)
						- invTimeHorizonObst * radius_ >= -RVOMath.RVO_EPSILON &&
						RVOMath.det(invTimeHorizonObst * relativePosition2 - orcaLines_[j].point, orcaLines_[j].direction) -
						invTimeHorizonObst * radius_ >= -RVOMath.RVO_EPSILON)
					{
						alreadyCovered = true;
				...
				else if (s > 1.0f && distSq2 <= radiusSq)
				{
                     ...
                     */
					if (RVOMath.det(relativePosition2, obstacle2.direction_) >= 0.0f)
					{
						line.point = new Vector2(0.0f, 0.0f);
				...
				if (RVOMath.det(leftLegDirection, -leftNeighbor.direction_) >= 0.0f)
				{
				...
				if (RVOMath.det(rightLegDirection, obstacle2.direction_) <= 0.0f)
				{
		...
		internal void update()
		{
			...
			AgentSimulationSystem.Instance.SetAgentVelocity(this.id_, velocity_ * Simulator.Instance.timeStep_);
         ...
         * <param name="agentRadius">The agentRadius of the circular constraint.</param>
