...
namespace FNZ.Client.View.UI.Component.Inventory
{
	public class LootableUI : MonoBehaviour
	{
		...
		public void Init(InventoryComponentClient inventory)
		{
			...
			m_inventorySize.x = inventory.m_Data.width;
			m_inventorySize.y = inventory.m_Data.height;
			BuildUI(m_inventorySize.x, m_inventorySize.y);
		...
		private void BuildUI(int width, int height)
		{
			...
			var parentT = (RectTransform) transform.parent;
			parentT.sizeDelta = new Vector2(width * CELL_WIDTH + 8, height * CELL_WIDTH + 8);
		...
		public InventoryComponentClient GetInventoryComp()
		{
			return m_Inventory;
		}
		...
		private void ReRender(List<int2> changedCells)
		{
			...
			RenderItems(m_inventorySize.x, m_inventorySize.y, changedCells);
			RenderCellColors(m_inventorySize.x, m_inventorySize.y);

			HB_Factory.DestroyHoverbox();

			if (m_HoveredCellIndex.x >= 0 && m_HoveredCellIndex.x < m_inventorySize.x && m_HoveredCellIndex.y >= 0 && m_HoveredCellIndex.y < m_inventorySize.y)
		...
		{
			Item clicked = m_Inventory.GetItemAtCell(x, y);
			if (m_Player.GetItemOnCursor() == null && clicked != null)
				UIManager.Instance.PlaySound(clicked.Data.pickupSoundRef);
		...
		public void OnCellMove()
		{
			if (m_HoveredCellIndex.x > -1)
			{
				...
				m_Placeable =
					topLeftX >= 0 &&
					topLeftY >= 0 &&
					topLeftX + item.Data.width <= m_inventorySize.x &&
					topLeftY + item.Data.height <= m_inventorySize.y;

				if (topLeftX < 0)
					topLeftX = 0;
				if (topLeftY < 0)
					topLeftY = 0;
				if (bottomRightX > m_inventorySize.x)
					bottomRightX = m_inventorySize.x;
				if (bottomRightY > m_inventorySize.y)
					bottomRightY = m_inventorySize.y;
