...
namespace FNZ.LevelEditor
{
	...
	public class FNSV1
	{
		...
		)
        {
			...
			foreach (GameObject go in edgeObjects)
			{
				...
				foreach (var pos in savedEdgeObjPositions)
				{
					...
					{
						Debug.LogWarning("Found multiple EdgeObjects at: " + pos.x + ":" + pos.y);
						cont = true;
						continue;
					}
				}
				...
				var mounted = ME_Control.GetChildMountedObject(go.transform);
				if (mounted)
				{
					IdTranslator.Instance.GenerateIdIfNeeded<MountedObjectData>(mounted.GetComponentInChildren<GameObjectIdHolder>().id);
				}
			...
			nb.Write((int) FileUtils.FNS_File_Version_Code.FNS_FILE_VERSION_1);
			...
			string fileName = folderPath + levelName;
			if (!levelName.Contains(LevelEditorUtils.SITE_FILE_ENDING))
				fileName += LevelEditorUtils.SITE_FILE_ENDING;
		...
		public static void WriteEdgeObjectSite(GameObject edgeObject, NetBuffer bw)
		{
			...
			var mounted = ME_Control.GetChildMountedObject(edgeObject.transform);

			bw.Write(mounted != null);
			if(mounted != null)
            {
				euler = mounted.transform.localRotation.eulerAngles;
				yRot = Mathf.Round(euler.y);

				id = mounted.GetComponent<GameObjectIdHolder>().id;
				bw.Write(IdTranslator.Instance.GetIdCode<MountedObjectData>(id));

				if (math.abs((int) yRot) == 180)
                {
					bw.Write(true);
				}
                else
		...
		)
		{
			...
			for (int x = 0; x < widthInTiles; x++)
			{
				for (int y = 0; y < heightInTiles; y++)
				{
                    ...
                    {
						ReadNullTileSite(x + startWorldX, y + startWorldY);
					}
				}
			}
		...
		private static void ReadNullTileSite(int worldX, int worldY)
		{
			...
			go.GetComponent<ME_Chunk>().NullMask[worldX % 32, worldY % 32] = true;
		}
		...
		private static GameObject CreateEdgeObject(string id, float worldX, float worldY, float rotation, NetBuffer br, IdTranslator idTransl)
		{
			...
			if (br.ReadBoolean())
			{
				id = idTransl.GetId<MountedObjectData>(br.ReadUInt16());
				var rotated = br.ReadBoolean();

				var mountedData = DataBank.Instance.GetData<MountedObjectData>(id);
				viewData = DataBank.Instance.GetData<FNEEntityViewData>(mountedData.viewVariations[0]);

				var newMounted = PrefabBank.GetInstanceOfMountedObjectPrefab(id);
				...
				newMounted.transform.SetParent(go.transform);

				newMounted.transform.localPosition = Vector3.zero;
				newMounted.transform.localRotation = Quaternion.Euler(0, rotated ? 180 : 0, 0);
			}
