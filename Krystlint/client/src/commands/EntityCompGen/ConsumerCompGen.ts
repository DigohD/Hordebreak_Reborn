...
let ConsumerCompPattern : DataDefGenConfig = {
	xsi: "ConsumerComponentData",
	tagHierarchy: [
		{name: "resources", default: "\n\t\t\t\t\t<ResourceConsumptionData>\n\t\t\t\t\t\t<resourceRef>resource_datadef_id</resourceRef>\n\t\t\t\t\t\t<amount>1</amount>\n\t\t\t\t\t</ResourceConsumptionData>\n\t\t\t\t"},
	]
}

export function ConsumerCompGen(editConsumer: TextEditorEdit){
	InsertDataComponentXML(ConsumerCompPattern, editConsumer);
}